...
# 2. Add this new function at the end of the file
async def convert_to_base64(text: str):
    """
    [ASYNC] Handles Win+B: Converts selected text to Base64 (and back if it is already Base64).
    """
    if state.app_controller_lock.locked():
        await safe_notification("System Busy", "Another task is in progress.")
        return
    ...
    async with state.app_controller_lock:
        text = text.strip()
        if not text:
            await safe_notification("Base64 Error", "No text selected.")
            return

        print(f"\n{Colors.BLUE}[Base64 Tool] Processing text...{Colors.ENDC}")

        try:
            # logic: Try to decode first. If it looks like valid Base64, decode it.
            ...
            try:
                ...
                # Check if it decodes to a readable utf-8 string
                decoded_string = decoded_bytes.decode('utf-8')
                result_text = decoded_string
                action_type = "Decoded"
            except Exception:
                # If decoding failed, it's likely plain text. Let's Encode it.
                encoded_bytes = base64.b64encode(text.encode('utf-8'))
                result_text = encoded_bytes.decode('utf-8')
                action_type = "Encoded"

            # Copy result to clipboard
            await asyncio.to_thread(pyperclip.copy, result_text)
            
            print(f"{Colors.GREEN}>> [{action_type}] Result copied to clipboard.{Colors.ENDC}")
            print(f"{Colors.CYAN}{result_text}{Colors.ENDC}")
            
            await safe_notification(f"Base64 {action_type}", "Result copied to clipboard.")

        except Exception as e:
            print(f"❌ {Colors.RED}[Base64 Error] Processing failed: {e}{Colors.ENDC}")
            await safe_notification("Base64 Error", "Conversion failed.")
